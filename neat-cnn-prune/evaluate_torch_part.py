...
class Net(nn.Module):
    ...
    def __init__(self, config, genome: neat3.genome.DefaultGenome):
                ...
                for idx_ in range(len(self.old_nodes[del_con[1]].in_nodes)):
                    ...
                    if self.old_nodes[del_con[1]].in_nodes[idx_] == del_con[0]:
                        ...
                        while j < len(self.old_nodes[del_con[1]].in_nodes):
                            ...
                            if self.old_nodes[del_con[1] ].layer % self.part == 0:
                                
                                self.old_nodes[ del_con[1] ].kernal[ j-1 ] = \
                                    self.old_nodes[ del_con[1] ].kernal[j ]
                            else:
                                self.old_nodes[ del_con[1] ].kernal1[ j-1 ] = \
                                    self.old_nodes[ del_con[1] ].kernal1[j ]                            
                            j += 1          
                        ...
                        if self.old_nodes[del_con[1] ].layer % self.part == 0:
                            ...
                            if len(self.old_nodes[del_con[1]].kernal1) > 1:                            
                                self.old_nodes[ del_con[1] ].kernal1 = \
                                    self.old_nodes[del_con[1]].kernal1[:-1]
                        ...
                        if self.old_nodes[del_con[1] ].layer % self.part == 0:
                            ...
                            assert  len(self.old_nodes[ del_con[1] ].in_nodes)== 0 or \
                                        len(self.old_nodes[del_con[1]].kernal1) == \
                                            len(self.old_nodes[ del_con[1] ].in_nodes)
...
#            to_prune_layer_idx))                
    ...
    def set_layers(self, genome: neat3.genome.DefaultGenome):
...
#                                 len(genome.layer[self.num_cnn_layer][1])
    ...
    def set_parameters(self, genome: neat3.genome.DefaultGenome):
            ...
            for j in range(len(l)):
             #   print('j', j, 'l[j] ', l[j])
                ...
                if i < self.num_cnn_layer:    
                    ...
                    else: a = np.array(self.old_nodes[l[j]].kernal1)
...
#                        print('i == 0', l[j],self.old_nodes[l[j]].in_nodes)
    ...
    #                            print('k', k, self.old_nodes[l[j]].in_nodes, genome.nodes[l[j]].in_nodes)
                            ...
                            for k in range(len(a)):
                                layer[i * 1 + 0 ].weight.data[j][nodes[ self.old_nodes[l[j]].in_nodes[k]][1] ] = \
                                       torch.FloatTensor(a[k].reshape(1, 1))      
                    #print(len(a), len(a[0]))
...
#                    torch.FloatTensor([genome.connections[(in_node, out_node)].weight])
    ...
    def write_back_parameters(self, genome: neat3.genome.DefaultGenome):
        ...
        for i in range(self.num_layer):
            ...
            for j in range(len(l)):
                ...
                if i < self.num_cnn_layer:
                    ...
                    else: a1 = np.array(self.old_nodes[l[j]].kernal1)    
                    ...
                    if len(self.old_nodes[l[j]].in_nodes) > 0:  
                        ...
                        if i % self.part == 0: 
                            ...
                            for k in range(len(a1)):
                                ...
                                genome.nodes[l[j]].kernal1[k] = (a[ nodes[ self.old_nodes[l[j]].in_nodes[k] ][1] ]).reshape(1)
