...
from neat3.activations import ActivationFunctionSet
from neat3.aggregations import AggregationFunctionSet
from neat3.config import ConfigParameter, write_pretty_params
from neat3.genes import DefaultConnectionGene, DefaultNodeGene
from neat3.graphs import creates_cycle
from neat3.six_util import iteritems, iterkeys
...
class DefaultGenomeConfig(object):
    ...
    def __init__(self, params):
        ...
        self._params = [ConfigParameter('num_inputs', int),
                        ...
                        ConfigParameter('part_size', int),  #  add
...
#            layer_num = randint(0, config.num_layer - 2)
        ...
        if layer_num == config.num_layer - 1:            
            ...
            if config.num_cnn_layer > layer_num + 1: # 下一层还是 cnn层
                ...
                if (layer_num + 1)% config.part_size == 0:
                    ...
                    if len(self.nodes[node_id].in_nodes) > 1:                   
                        tmp = randn(1, len(self.nodes[node_id].kernal1[0]))                            
                        self.nodes[node_id].kernal1 += tmp.tolist()   
                             
                    if len(self.nodes[node_id].kernal1) != len(self.nodes[node_id].in_nodes):                    
                        print(len(self.nodes[node_id].kernal1) ,' != ', len(self.nodes[node_id].in_nodes))
                    assert len(self.nodes[node_id].kernal1) == len(self.nodes[node_id].in_nodes)                  
        ...
        for key in connections_to_delete:
            if key[0] == del_key: # del_key是 输入
                ...
                if self.nodes[key[1]].layer % config.part_size == 0:
                    ...
                    self.nodes[del_key].kernal1 = self.nodes[del_key].kernal1[:1]                         
...
#            print(num_enabled_connections, ' ',len(self.connections))
    ...
    def connect_full(self, config):
        ...
        for node_id, in_num in in_channels.items(): # for each cnn node
            if in_num < 2:
                continue
            if self.nodes[node_id].layer% config.part_size == 0:
                # 补全 (in_num, len(kernal[0]))
                ...
                assert len(self.nodes[node_id].kernal) == len(self.nodes[node_id].in_nodes)                                
                 ...
                 # 补全 (in_num, len(kernal1[0]))
                tmp = randn(in_num-1, len(self.nodes[node_id].kernal1[0])) 
...
#                    print('input_id' ,in_id , output_id)
        ...
        for node_id, in_num in in_channels.items():            
            ...
            if self.nodes[node_id].layer% config.part_size == 0:      
                ...
                self.nodes[node_id].kernal1 += tmp.tolist()    
                if len(self.nodes[node_id].kernal1) != len(self.nodes[node_id].in_nodes):                
                    print(len(self.nodes[node_id].kernal1), ' ', len(self.nodes[node_id].in_nodes),' ', in_num )
                assert len(self.nodes[node_id].kernal1) == len(self.nodes[node_id].in_nodes)   
