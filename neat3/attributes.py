...
from neat3.config import ConfigParameter
from neat3.six_util import iterkeys, iteritems
...
#                                                                        self.init_type_name))
        ...
        # 这里是先初始化一个list，即 config.input_channels = 1 的情况
        for in_channel in range(1):            
            value.append([]) 
            
            for i in range(config.kernal_size):
                if ('gauss' in init_type) or ('normal' in init_type):
                    value[in_channel].append(self.clamp(gauss(mean, stdev), config))
                elif 'uniform' in init_type:
                    min_value = max(getattr(config, self.min_value_name),
                                    (mean - (2 * stdev)))
                    max_value = min(getattr(config, self.max_value_name),
                                    (mean + (2 * stdev)))
                    value[in_channel].append(uniform(min_value, max_value))
                else:
                    raise RuntimeError("Unknown init_type {!r} for {!s}".format(getattr(config,
                                                                            self.init_type_name),
                                                                            self.init_type_name))
...
#                continue
            for i in range(config.kernal_size):
                r = random()
                if r < mutate_rate:
                    value[in_channel][i] = self.clamp(value[in_channel][i] + gauss(0.0, mutate_power), config)
                elif r < replace_rate + mutate_rate:
                    value[in_channel][i] =  self.init_single_value(config)
...
class ListAttribute1(BaseAttribute):
    """
    Class for numeric attributes,
    such as the response of a node or the weight of a connection.
    """
