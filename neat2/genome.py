...
from neat2.six_util import iteritems, iterkeys


class DefaultGenomeConfig(object):
    """Sets up and holds configuration information for the DefaultGenome class."""
    allowed_connectivity = ['unconnected', 'fs_neat2_nohidden', 'fs_neat2', 'fs_neat2_hidden',
...
#                continue
        ...
        # fs_neat2 is not used in cnn
        """
        if 'fs_neat2' in config.initial_connection:
            if config.initial_connection == 'fs_neat2_nohidden':
                self.connect_fs_neat2_nohidden(config)
            elif config.initial_connection == 'fs_neat2_hidden':
                self.connect_fs_neat2_hidden(config)
            else:
                if config.num_hidden > 0:
                    print(
                        "Warning: initial_connection = fs_neat2 will not connect to hidden nodes;",
                        "\tif this is desired, set initial_connection = fs_neat2_nohidden;",
                        "\tif not, set initial_connection = fs_neat2_hidden",
                        sep='\n', file=sys.stderr);
                self.connect_fs_neat2_nohidden(config)
...
#            layer_num = randint(0, config.num_layer - 2)
    ...
    def connect_fs_neat2_hidden(self, config):
        """
